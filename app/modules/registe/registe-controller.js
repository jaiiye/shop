// registe-controller.js
(function () {
	function registeControll($scope, $locaton){
		console.log('registe...');
		...
		$scope.checkname= function() {
			...
			else if(isNull($scope.username) || typeof($scope.username) == "undefined"){
				console.log('username',$scope.username);
				$scope.username_msg = '用户名不能为空';
			}
			else{
				console.log('username',$scope.username);
				$scope.username_msg = '';
	...
	registeControll.$inject = ['$scope', '$location'];
	angular.module('luna').controller('registeController',registeControll);
})();